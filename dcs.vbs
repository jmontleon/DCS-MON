...
Sub ProcessMonitor (path, window_x, window_y, multiplayer_button_x, multiplayer_button_y, new_server_button_x, new_server_button_y, start_button_x, start_button_y)
  running = false
  ...
  if (not running) then
    command = chr(34) & hot_key_path & hot_key_script & chr(34) & " " & dcs_user & " " & dcs_pass & " " & chr(34) & path & chr(34) & " " & window_x & " " & window_y & " " & multiplayer_button_x & " " & multiplayer_button_y & " " & new_server_button_x & " " & new_server_button_y & " " & start_button_x & " " & start_button_y
    Shell.Run command, 0, True
    WScript.Sleep (60000)
  end if
end Sub
...
while true
  if release_monitor = true then
    ...
    ProcessMonitor release_path, release_window_x, release_window_y, release_multiplayer_button_x, release_multiplayer_button_y, release_new_server_button_x, release_new_server_button_y, release_start_button_x, release_start_button_y
  End If

  if alpha_monitor = true then
    PortMonitor alpha_port, alpha_path
    ProcessMonitor alpha_path, alpha_window_x, alpha_window_y, alpha_multiplayer_button_x, alpha_multiplayer_button_y, alpha_new_server_button_x, alpha_new_server_button_y, alpha_start_button_x, alpha_start_button_y
  End If

  if beta_monitor = true then
    PortMonitor beta_port, beta_path
    ProcessMonitor beta_path, beta_window_x, beta_window_y, beta_multiplayer_button_x, beta_multiplayer_button_y, beta_new_server_button_x, beta_new_server_button_y, beta_start_button_x, beta_start_button_y
  End If
